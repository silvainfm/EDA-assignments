...
g1 = ggplot(subset(df, country %in% c("Brazil", "Russia", "China", "India", "China", "South Africa")),
       aes(x = year,
           y = gdpPercap)) +
  geom_line(aes(color=country), alpha = 0.5) +
  geom_point(alpha = 0.8,
             size = 0.4) +
  theme_light() +
  labs(title = "GDP per capita of the BRICS",
       x = "Year",
       y = "GDP per capita") +
  scale_x_continuous(breaks = seq(1950,2000, 25)) + 
...
**Everything on the same graph is better as you can compare each country, but the graph looks a little busy. 4 different ones makes it small and makes the image seem very busy/overcrowded, also it makes it seem like India and South Africa are complementing each other.The facet wrap makes sense when the 4 variables differ largely.**
...
**I think that facet wrap works much better than facet grid in this case. It is difficult to compare the plots, as they are split and have the same scales.**
...
```
**Scale color brewer applies the color palette specified in the parameters to the aesthetic specified in the color arg - that's how it knows where to apply color.**
...
frequency(eu_stocks)
```
**It is how often the sample is taken per unit of time.**
...
```
**The mean over time does not seem constant, this implies that the time series is not stationary.**
